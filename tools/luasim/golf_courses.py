...
def cmd_build(args):
    ...
    # the players' portraits, when the repository has them (golf/players_photos.lua:
    # the owner's request of 2026-09-24 put them in the public gallery)
    photos = HERE / "golf" / "players_photos.lua"
    pblock = ""
    if photos.exists():
        t = photos.read_text(encoding="utf-8")
        pblock = t[t.index("PHOTOS = {"):]
    ...
    for d in docs:
        ...
        head = ["-- @upload-only\n",
                title + "eighteen real holes in four minutes.\n",
                f'-- Generated by tools/luasim/golf_courses.py build from golf_course.lua; edit that, not this.\n']
        if pblock:
            head.insert(1, "-- @photo people, the players' portraits, published at the owner's request (2026-09-24)\n")
        head.append(f'COURSE = "{d["id"]}"\n')
        if pblock:
            head.append(pblock)
