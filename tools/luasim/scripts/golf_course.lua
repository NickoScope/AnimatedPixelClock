-- @upload-only
-- GOLF COURSE - eighteen real holes of stroke play in four minutes, a new round every run.
--
...
--
-- A round is made at the start of each four-minute period from the clock, so
...
PERIOD = 240                                -- four minutes a round (the owner, 2026-09-24: slower, it flickered at two)
FPS = 15

local W, H = px.size()
local floor, sqrt, min, max, abs = math.floor, math.sqrt, math.min, math.max, math.abs

local INTRO, HOLE_S, HOLES = 8.0, 12.2, 18
...
local RESULT_AT = INTRO + HOLE_S * HOLES          -- 227.6 s; the result to 240
...
-- ---------------------------------------------------------------- the clock
-- Words over the picture with a dark shadow under them, not on a black box:
-- the boxes covered most of the screen (the owner, 2026-09-24).
local BLACK = {0, 0, 0}
local function ST(x, y, s, c, font)
  T(x + 1, y + 1, s, BLACK, font)
  T(x, y, s, c, font)
end
...
-- ---------------------------------------------------------------- on screen
-- 5x7: the small font's Д reads as А (УДАР as УААР), and captions are full of Д
local function label(s, pl)
  ST(1, H - 9, s, COL[pl], "5x7")
end

local function hole_card(g, h, a)
  ST(2, 10, "ЛУНКА " .. h.n .. "  ПАР " .. h.par, {255, 215, 90}, "5x7")
  ST(2, 19, h.len .. " М  ИНДЕКС " .. h.si, {230, 230, 230}, "5x7")
end
...
local FLY_END, TEE_END, PLAY_END = 2.0, 4.8, 10.0
local SWING_S, FLIGHT_S = 0.9, 1.6
...
local function scene_ferry(g, h, st, u)
  ...
  ST(1, H - 7, "ПАРОМ ЧЕРЕЗ " .. st.river, {255, 215, 90})
...
local function minimap(h, pos, flying, holed)
  ...
  for i = 1, #h.pts - 1 do                 -- the line of play, over its shadow
    local ax, ay = at(h.pts[i][1], h.pts[i][2])
    local bx, by = at(h.pts[i + 1][1], h.pts[i + 1][2])
    px.line(floor(ax) + 1, floor(ay) + 1, floor(bx) + 1, floor(by) + 1, 0, 0, 0)
    px.line(floor(ax), floor(ay), floor(bx), floor(by), 150, 230, 120)
  end
...
local function scene_ace(g, h, st, ht)
  ...
  local s = "HOLE IN ONE!"
  if (u * 4) % 1 < 0.75 then
    ST((W - TW(s, "5x7")) // 2, 27, s, {255, 215, 60}, "5x7")
  end
...
function draw()
  frame_no = frame_no + 1
  local n = px.now()
  -- the round of this period: periods start on minutes divisible by its length
  local mins = PERIOD // 60
  local id = ((n.year % 100) * 400 + n.yday) * (1440 // mins) + (n.hour * 60 + n.min) // mins
  if id ~= round_id then
