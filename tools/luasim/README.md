...
| `px.button()` | since 2.7.2: how many times the effect's button has been pressed - the knob's click or the remote's OK on its page, or `POST /api/lua {"click":true}`. A count, not a state: react when it changes. In luasim and fxhost, `--clicks f1,f2,...` presses it at those frames |
