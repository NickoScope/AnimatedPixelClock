...
with tempfile.TemporaryDirectory() as d:
    ...
    try:
        ...
        # a photograph: refused unless said to have no person in it, then tagged
        PH = ROOT / "tools/luasim/scripts/zz_gallery_test_ph.lua"
        PH.write_text("-- Generated by tools/luasim/photo_to_lua.py (test)\nfunction draw()\n"
                      "  px.rect(0, 0, 128, 64, 40, 120, 200, true)\nend\n")
        try:
            r = call(M.gallery_publish, M.GalleryPublishIn, name="zz_gallery_test_ph", about=ABOUT)
            check(r.startswith("Refused") and "no person" in r, "a photograph without photo_no_people: refused")
            r = json.loads(call(M.gallery_publish, M.GalleryPublishIn, name="zz_gallery_test_ph", about=ABOUT,
                                photo_no_people=True))
            head = git("show", "gallery-staging:gallery/zz_gallery_test_ph.lua", cwd=pi).splitlines()[:3]
            check(r.get("ok") and "-- @photo no-people" in head, "with photo_no_people: published and tagged")
        finally:
        ...
        check(git("cat-file", "-e", "main:gallery/zz_gallery_test_ph.lua", cwd=gh) == "" and
              "-- @photo no-people" in git("show", "main:gallery/zz_gallery_test_ph.lua", cwd=gh),
              "sync carries the tagged photograph")
        ...
        call(M.gallery_unpublish, M.GalleryRemoveIn, name="zz_gallery_test_ph")
        board2 = "# Screen of the Day\n\nNothing yet.\n"
        call(M.gallery_scoreboard, M.ScoreboardIn, markdown=board2)
        out = sync()
        check("removed " + G.shown(STEM) in git("log", "-1", "--format=%s", "main", cwd=gh) and
              "removed ZZ GALLERY TEST PH" in git("log", "-1", "--format=%s", "main", cwd=gh), "sync: removed on GitHub too")
