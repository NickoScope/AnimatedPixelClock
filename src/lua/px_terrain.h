...
static void pxt_rgb(lua_State *L, int t, const char *k, float out[3], float r, float g, float b) {
  ...
  if (lua_istable(L, -1)) {
    ...
    for (int c = 0; c < 3; c++) {
      lua_rawgeti(L, i, c + 1);
      if (lua_isnumber(L, -1)) out[c] = (float)lua_tonumber(L, -1);
      lua_pop(L, 1);
    }
  }
  lua_pop(L, 1);
...
static const unsigned char *pxt_str(lua_State *L, int t, const char *k, size_t *len) {
  ...
  if (lua_type(L, -1) == LUA_TSTRING) s = (const unsigned char *)lua_tolstring(L, -1, len);
  lua_pop(L, 1);          // the string stays alive: the grid table still holds it
  return s;
}

static inline int pxt_mod(int v, int m) { int r = v % m; return r < 0 ? r + m : r; }
static inline float pxt_clamp(float v, float a, float b) { return v < a ? a : v > b ? b : v; }

static int px_terrain_lua(lua_State *L, unsigned char *fb, int fbw, int fbh) {
  luaL_checktype(L, 1, LUA_TTABLE);
  const int T = 1;

  // ---- the grid
  lua_getfield(L, T, "grid");
  luaL_argcheck(L, lua_istable(L, -1), 1, "grid must be a table");
  const int G = lua_gettop(L);
  const int nx = (int)pxt_num(L, G, "w", 0), ny = (int)pxt_num(L, G, "h", 0);
  const float cell = pxt_num(L, G, "cell", 1), gx0 = pxt_num(L, G, "x0", 0), gy0 = pxt_num(L, G, "y0", 0);
  const float hbase = pxt_num(L, G, "hbase", 0), hscale = pxt_num(L, G, "hscale", 0.05f);
  ...
  luaL_argcheck(L, nx >= 2 && ny >= 2 && nx <= 1024 && ny <= 1024 && cell > 0.01f, 1, "grid size");
  ...
  // ---- the camera
  lua_getfield(L, T, "cam");
  luaL_argcheck(L, lua_istable(L, -1), 1, "cam must be a table");
  const int CI = lua_gettop(L);
  PxTerrainCam cam = {pxt_num(L, CI, "x", 0), pxt_num(L, CI, "y", 0), pxt_num(L, CI, "z", 2),
                      pxt_num(L, CI, "yaw", 0), pxt_num(L, CI, "hor", fbh / 2), pxt_num(L, CI, "f", 100)};
  ...
  const float fog0 = pxt_num(L, T, "fog0", 140), fogr = pxt_clamp(pxt_num(L, T, "fogr", 900), 1, 1e6f);
  const float farh = pxt_num(L, T, "farh", 4);
  const int colw = (int)pxt_clamp(pxt_num(L, T, "colw", 1), 1, 8);
  const float step = pxt_clamp(pxt_num(L, T, "step", 1.028f), 1.002f, 1.5f);
  const float zfar = pxt_clamp(pxt_num(L, T, "zfar", 900), 1, 5000);
  const float grass = pxt_num(L, T, "grass", 60);
  const int frame = (int)pxt_num(L, T, "frame", 0);
  ...
  for (int col = 0; col < fbw; col += colw) {
    ...
    while (z < zfar) {
      ...
      const int i = (int)floorf(fi), j = (int)floorf(fj);
      float r, g, b, h;
      int water = 0;
      if (i >= 0 && j >= 0 && i < nx - 1 && j < ny - 1) {
        ...
        if (pat == 4) {
          ...
          if (pat == 1) l *= (pxt_mod((int)floorf(x / per), 2) == 0) ? 1 + amt : 1 - amt * 0.86f;
          else if (pat == 2) l *= (pxt_mod((int)floorf(x / per) + (int)floorf(y / per), 2) == 0) ? 1 + amt : 1 - amt * 0.8f;
          else if (pat == 3) l *= (1 - amt) + 2 * amt * (pxt_mod(i * 7 + j * 13, 11) / 10.0f);
          if (light) l *= light[idx] / 128.0f;
          if (z < grass) l *= 0.93f + 0.14f * (pxt_mod((int)floorf(x * 3) * 73 + (int)floorf(y * 3) * 151, 17) / 16.0f);
        ...
        const int hx = (int)floorf(x / 7), hy = (int)floorf(y / 7);
        h = farh + 10 * u + 1.5f * (pxt_mod(hx * 5 + hy * 3, 7) / 6.0f);
        const float l = 0.85f + 0.25f * (pxt_mod(hx * 7 + hy * 13, 5) / 4.0f);
      ...
      if (syf < yb) {
        if (water) {                       // the sky in the water, more of it far away
          ...
          if (pxt_mod(i * 7 + j * 13 + frame, 37) == 0) { r = 200; g = 225; b = 250; }
        }
