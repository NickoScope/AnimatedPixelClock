...
| `px.button()` (2.7.2) | a counter, no cost | the knob's click and the remote's OK on an effect page, and `POST /api/lua {"click":true}`; a script compares it with the last value it saw (OCEANARIUM: 1 click lights, 2 demo, 3 now) |
