...
- One custom font ships: `PicopixelFB` (`src/fonts/picopixel_fb.h`), stock
  ...
  Beside it, `PicopixelCyr` covers U+0400..U+045F, generated by
  `tools/fonts/mkcyr.py`. **`display.print()` cannot draw it**: Adafruit GFX
  writes one byte at a time. Text that may hold Cyrillic goes through
  `src/fonts/pxfb_text.h` (`pxfbDraw`, `pxfbWidth`), as px.text and the world
  clock do. Unknown code points draw a solid block, not a space.
